...
const FooterPageEditor = () => {
    ...
    const fetchContent = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await adminFetch('/site-content/pages/footer');
            if (response.ok) {
    ...
    const handleSeedContent = async () => {
        try {
            setSeeding(true);
            setError(null);
            const response = await adminFetch('/site-content/seed/footer', {
                method: 'POST'
            });
            if (response.ok) {
                await fetchContent();
        ...
        return (
            <div className="flex items-center justify-center h-screen bg-slate-50">
                <div className="text-center">
                    ...
                    <p className="text-slate-500">Loading Footer content...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="h-screen flex flex-col bg-slate-100">
                <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-4 shrink-0">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-800">Footer</h1>
                        <p className="text-xs text-slate-500">Edit footer content, links, and contact info</p>
                    </div>
                </header>
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="text-center max-w-md">
                        ...
                        <p className="text-slate-500 mb-8">
                            The Footer doesn't have any content in the database.
            ...
            <div className="flex-1 flex overflow-hidden">
                {(activeTab === 'edit' || activeTab === 'split') && (
                    <div className={`${activeTab === 'split' ? 'w-1/2' : 'w-full'} overflow-y-auto bg-slate-50 border-r border-slate-200`}>
                        <FooterEditor content={content} onUpdate={handleContentUpdate} />
                    </div>
                )}
