...
const AcademicsPageEditor = () => {
    ...
    const fetchContent = async () => {
        try {
            setLoading(true);
            const response = await adminFetch('/site-content/pages/academics');
            if (response.ok) {
        ...
        return (
            <div className="flex items-center justify-center h-screen bg-slate-50">
                <div className="text-center">
                    ...
                    <p className="text-slate-500">Loading Academics page content...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="h-screen flex flex-col bg-slate-100">
            ...
            <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-800">Academics Page</h1>
                        <p className="text-xs text-slate-500">Edit hero, curriculum, departments, and methodology</p>
                    </div>
                </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                {(activeTab === 'edit' || activeTab === 'split') && (
                    <div className={`${activeTab === 'split' ? 'w-1/2' : 'w-full'} overflow-y-auto bg-slate-50 border-r border-slate-200`}>
                        <AcademicsEditor content={content} onUpdate={handleContentUpdate} />
                    </div>
                )}
                ...
                {(activeTab === 'preview' || activeTab === 'split') && (
                    <div className={`${activeTab === 'split' ? 'w-1/2' : 'w-full'} overflow-y-auto bg-white`}>
                        <AcademicsPreview content={content} />
                    </div>
                )}
            </div>
        </div>
